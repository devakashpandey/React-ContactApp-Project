...
const AddEditPage = () => {
  ...
  useEffect(() => {
    if (id) {
      // if we have the id which means user perform the update operation
      setState({ ...data[id] });
    } else {
    ...
    if (!name || !email || !contact) {
      ...
      if (!id) {
        const result = await fetch(
          "https://react-contact-app-6f23b-default-rtdb.firebaseio.com/.json",
          {
            method: "POST",
            headers: {
  ...
  return (
    <>
      ...
      <div className="custom-shape-divider-bottom-1677334060">
        <svg
          data-name="Layer 1"
